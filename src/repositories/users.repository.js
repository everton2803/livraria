...
class UsersRepository {
    ...
    async create({ username, passwordHash, name, email }) {
        const result = await db.run('INSERT INTO users (username, password_hash, name, email) VALUES (?, ?, ?, ?)', [username, passwordHash, name, email]);
        console.log(result);
    }
}
